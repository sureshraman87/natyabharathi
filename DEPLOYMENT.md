...
| YouTube/Vimeo lesson video shows a player error instead of playing | Usually a `Referrer-Policy` problem: the app already sends `strict-origin-when-cross-origin` (both as a response header and a `referrerpolicy` attribute on the `<iframe>`), but **if you're proxying through Cloudflare**, some of its security presets rewrite response headers and force `Referrer-Policy: same-origin`, which YouTube/Vimeo can reject as an embedding-validation failure. Check the header reaching the browser with `curl -sI https://natyabharathi.sureshraman.com/ \| grep -i referrer-policy`; if it doesn't say `strict-origin-when-cross-origin`, add a Cloudflare **Response Header Transform Rule** that sets `Referrer-Policy: strict-origin-when-cross-origin` to override it. |
